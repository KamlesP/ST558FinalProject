      ...
      tabItem(tabName = "explore",
              fluidRow(
                ...
                box(title = h4('Corr Plots', style = 'font-family: Times New Roman; text-align:center'), width = 6, 
                    ...
                    background = 'maroon'),
                box(title = h4('Plot Interpretation', style = 'font-family: Times New Roman; text-align:center'), width = 6, uiOutput('text1'), tableOutput('table1'))
              ),
              fluidRow(
                 ...
                 fluidRow(
                   tabBox(
                     ...
                     tabPanel(title = h4('Modeling Info', style = 'font-family: Times New Roman; background-color:lighblue; font-weight: bold; font-size:3vw'), 
                              fluidRow(
                                box(title = h5('GLM (Generalized Linear Model)', style = 'color:black;margin-top: 10px; font-family: Times New Roman;font-weight: bold; text-align: center; font-size:1.5vw' ), width = 12, uiOutput('glm'), status = 'success', solidHeader = T, collapsible = T),
                                box(title = h5('Classification Tree', style = 'color:black;margin-top: 10px; font-family: Times New Roman;font-weight: bold; text-align: center; font-size:1.5vw'), width = 12, status = 'success', uiOutput('cart'), solidHeader = T, collapsible = T),
                                box(title = h5('Random Forest', style = 'color:black;margin-top: 10px; font-family: Times New Roman;font-weight: bold; text-align: center; font-size:1.5vw'), width = 12, status = 'success', solidHeader = T, collapsible = T, uiOutput('rf'))
                              )
                              ),
                     ...
                     tabPanel(title  = h4('Prediction', style = 'font-family: Times New Roman; background-color:lighblue; font-weight: bold; font-size:3vw'),
                              fluidRow(
                                box(width = 4,title = p('Variables for Prediction', style = 'font-family:Times New Roman; text-align:center; font-weight:bold'),
                                    uiOutput('note'),
                                    verbatimTextOutput('selectedvar'),
                                    actionButton('show',  "Show User data"),
                                    actionButton('predict', "Predict"),
                                    ), 
                                box( solidHeader = T, collapsible = T, background = 'orange', width = 4,
                                    numericInput(inputId = 'fa',
                                                 label = 'Fixed Acidity',
                                                 value = 0),
                                    actionButton('s_afa', "Submit"),
                                    numericInput(inputId = 'va',
                                                 label = 'Volatile Acidity',
                                                 value = 0),
                                    actionButton('s_va', "Submit"),
                                    numericInput(inputId = 'ca',
                                                 label = 'Citric Acid',
                                                 value = 0),
                                    actionButton('s_ca', "Submit"),
                                    numericInput(inputId = 'rs',
                                                 label = 'Residual Sugar',
                                                 value = 0),
                                    actionButton('s_rs', "Submit"),
                                    numericInput(inputId = 'ch',
                                                 label = 'Chloride',
                                                 value = 0),
                                    actionButton('s_ch', "Submit")),
                                box(width = 4, collapsible = T, background = 'orange',
                                    numericInput(inputId = 'so2',
                                                 label = 'Free Sulphur Di-oxide',
                                                 value = 0),
                                    actionButton('s_so2', "Submit"),
                                    numericInput(inputId = 'tso2',
                                                 label = 'Total Free Sulphur Di-oxide',
                                                 value = 0),
                                    actionButton('s_tso2', "Submit"),
                                    numericInput(inputId = 'rho',
                                                 label = 'Density',
                                                 value = 0),
                                    actionButton('s_rho', "Submit"),
                                    numericInput(inputId = 'pH',
                                                 label = 'pH',
                                                 value = 0),
                                    actionButton('s_pH', "Submit"),
                                    numericInput(inputId = 'sp',
                                                 label = 'Sulphates',
                                                 value = 0),
                                    actionButton('s_sp', "Submit"),
                                    numericInput(inputId = 'alcohol',
                                                 label = 'Alcohol',
                                                 value = 0),
                                    actionButton('s_alcohol', "Submit")
                                ),
                                box(collapsible = T, solidHeader = T, width = 8,
                                    title = p('User Input Data', style = "color:red;font-family: Times New Roman;font-weight:bold; text-align:center "),
                                  tableOutput("userTable")),
                                box(collapsible = T, solidHeader = T, background = 'orange', width = 8,
                                    title = p('Prediction', style = "color:black;font-family: Times New Roman;font-weight:bold; text-align:center "),
              ...
              h4('Dataset', icon("database",lib = "font-awesome"),style="font-family:Times New Roman;color:black;text-align:center; font-size:1.5vw",
                 fluidRow(
                          column(DT::dataTableOutput("WineData"),
                                 width = 12)),
                 hr(),
                 p(em("Made by"),br("KayP(kpandey@ncsu.edu)"),br("ST558 Project"),style="text-align:center; font-family: times")
                 )
      )
    )
  )
